import { forwardRef, useEffect, useRef } from 'react';
import { gsap } from 'gsap';
import { useTheme } from '../../context/ThemeContext';

const Textarea = forwardRef(({
  label,
  error,
  className = '',
  rows = 4,
  ...props
}, ref) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const wrapRef    = useRef(null);
  const labelRef   = useRef(null);
  const dividerRef = useRef(null);

  useEffect(() => {
    const wrap = wrapRef.current;
    if (!wrap) return;

    /* Entrance — same pattern as Skills sub-text */
    gsap.fromTo(wrap,
      { y: 20, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.8, ease: 'expo.out', delay: 0.05 }
    );

    /* Underline: scaleX expand on mount (mirrors group-divider) */
    if (dividerRef.current) {
      gsap.fromTo(dividerRef.current,
        { scaleX: 0, transformOrigin: 'left center' },
        { scaleX: 1, duration: 0.9, ease: 'expo.out', delay: 0.1 }
      );
    }
  }, []);

  /* Focus / blur interactions */
  const handleFocus = () => {
    if (labelRef.current) {
      gsap.to(labelRef.current, { x: 4, duration: 0.3, ease: 'expo.out' });
    }
  };
  const handleBlur = () => {
    if (labelRef.current) {
      gsap.to(labelRef.current, { x: 0, duration: 0.35, ease: 'expo.out' });
    }
  };

  return (
    <div ref={wrapRef} className="w-full">
      {label && (
        <label
          ref={labelRef}
          className={`
            block mb-2 ml-1 transition-colors duration-300
            ${isDark ? 'text-slate-500' : 'text-slate-400'}
          `}
          style={{
            fontFamily: "'DM Mono', monospace",
            fontSize: '0.6rem',
            fontWeight: 300,
            letterSpacing: '0.22em',
            textTransform: 'uppercase',
          }}
        >
          {label}
        </label>
      )}

      <div className="relative">
        <textarea
          ref={ref}
          rows={rows}
          onFocus={handleFocus}
          onBlur={handleBlur}
          className={`
            w-full px-5 py-3.5 rounded-2xl outline-none resize-none custom-scrollbar
            transition-all duration-300
            ${!isDark
              ? `bg-slate-100 text-slate-700 placeholder:text-slate-400
                 border border-transparent
                 shadow-[inset_2px_2px_6px_rgba(163,177,198,0.4),inset_-2px_-2px_6px_rgba(255,255,255,0.8)]
                 focus:bg-white
                 focus:shadow-[0_0_0_2px_rgba(59,130,246,0.5),0_10px_20px_-5px_rgba(59,130,246,0.15)]`
              : `bg-[#0f172a] text-slate-200 placeholder:text-slate-600
                 border border-slate-800
                 shadow-[inset_2px_2px_4px_rgba(0,0,0,0.6),inset_-1px_-1px_3px_rgba(255,255,255,0.05)]
                 focus:border-blue-500/50
                 focus:shadow-[0_0_0_1px_rgba(59,130,246,0.5),0_0_15px_rgba(59,130,246,0.2)]`
            }
            ${error ? '!border-red-500 !shadow-[0_0_0_1px_rgba(239,68,68,0.5)]' : ''}
            ${className}
          `}
          style={{ fontFamily: "'DM Mono', monospace", fontWeight: 300, fontSize: '0.88rem', letterSpacing: '0.02em' }}
          {...props}
        />
      </div>

      {/* Ruled underline (mirrors group-divider scaleX) */}
      <div
        ref={dividerRef}
        style={{
          marginTop: '0.5rem',
          marginLeft: '0.25rem',
          height: '1px',
          background: error
            ? 'rgba(239,68,68,0.6)'
            : isDark
              ? 'linear-gradient(to right, rgba(255,255,255,0.08), transparent)'
              : 'linear-gradient(to right, rgba(0,0,0,0.07), transparent)',
        }}
      />

      {error && (
        <p
          className="mt-1.5 ml-1 text-sm text-red-500 font-medium flex items-center gap-1"
          style={{ fontFamily: "'DM Mono', monospace", fontWeight: 300, fontStyle: 'italic', fontSize: '0.72rem', letterSpacing: '0.04em' }}
        >
          <span className="w-1 h-1 rounded-full bg-red-500 inline-block animate-pulse" />
          {error}
        </p>
      )}
    </div>
  );
});

Textarea.displayName = 'Textarea';
export default Textarea;